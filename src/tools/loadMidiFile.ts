...
import { LoadMidiFileArgs, LoadMidiFileResult } from '../types/tools.js';
...
export class LoadMidiFileTool {
  private static loadedFiles = new Map<string, LoadedMidiFile>();
  ...
  static async execute(args: LoadMidiFileArgs): Promise<LoadMidiFileResult> {
    try {
      ...
      const tracks = midiFile.tracks.map((_, index) =>
        MidiAnalyzer.analyzeTrack(midiFile, index)
      );

      // Store loaded file
      const loadedFile: LoadedMidiFile = {
        id: fileId,
  ...
  static getLoadedFile(fileId: string): LoadedMidiFile | undefined {
    return this.loadedFiles.get(fileId);
  }

  static getLoadedFileByPath(filePath: string): LoadedMidiFile | undefined {
  ...
  static getAllLoadedFiles(): LoadedMidiFile[] {
    return Array.from(this.loadedFiles.values());
  }

  static removeLoadedFile(fileId: string): boolean {
